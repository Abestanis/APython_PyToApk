...
class ApkTemplateFiller(object):
    ...
    FORMAT_ARG_VERIFIERS = {
        ...
        'minPyVersion': lambda val: val == None or ApkTemplateFiller._PYTHON_VERSION_REGEX.search(val) != None,
                                    # any Python version or None
        'requirements': lambda val: type(val) == str, # any string
        'appId': lambda val: ApkTemplateFiller._JAVA_PACKAGE_REGEX.search(val) != None, # any valid java package path
    ...
    TEMLATE_ICON_PATH = 'app/src/main/res/drawable-mdpi/app_launcher_icon.png'.replace('/', os.path.sep)
    ...
    def validateValues(self):
        ...
        for key in self.formatArgs:
            if key not in self.FORMAT_ARG_VERIFIERS:
                self.logger.warn('Found an unknown formatting argument: "' +
                                 argName + '"')
        ...
        if self.appManifestTemplate != None and not os.path.isfile(self.appManifestTemplate):
            valid = False
    ...
    def loadConfigFile(self, path):
        ...
        if parser.has_section('android_app'):
            if self.formatArgs is None: self.formatArgs = {}
            ...
            if parser.has_option('android_app', 'app_manifest_template'):
                self.appManifestTemplate = resolvePath(parser.get('android_app', 'app_manifest_template'), appDir)
            if len(self.formatArgs) == 0:
                if self.appIcon == None and self.appManifestTemplate == None:
    ...
    def fillTemplate(self, sdkPath):
        ...
        if self.formatArgs is None:
            ...
            return True
        if not self.validateValues(): return False
        if not (self.copyCustomManifestTemplate() and self.copyIcon()): return False
